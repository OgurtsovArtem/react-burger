...
const OrderCard: FC<IProfileOrderProps> = ({data}) => {
  ...
  const statusColor = 'wait' === status ? styles.status_wait : 'done' === status ? styles.status_done : styles.status_reject;
  const statusText = 'pending' === status ? "Готовится" : 'done' === status ? "Выполнен" : "Отменён";
  const countImages = ingredients.length > 5 ? ingredients.length - 6 : '';

  return (
    <Link to={{ pathname: `/feed/${_id}`, state: { background: location } }}>
      <div className={`${styles.card} pt-6 pb-6 pl-6 pr-6 mb-4`}>
        ...
        <div className={styles.body}>
          <div className={styles.icons}>
            {images.slice(0, 6).map((item: any, index: Key | null | undefined) => {
              return(
